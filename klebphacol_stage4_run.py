...
def print_metrics_table(results, label):
    print(f"\n  [{label}]")
...
def main():
    ...
    if os.path.exists(s4.MODEL_PATH):
        ...
        # still re-run the exclusion guard against the current exclusion list
    ...
    for label, path in INTERACTION_FILES.items():
        ...
        print("=" * 66)
        res_full, merged_full = s4.evaluate(pred_full, path, phage_stratum)
        res_sub, merged_sub = s4.evaluate(pred_sub, path, phage_stratum)
        ...
        print("\n  Sensitivity delta (full -> subsampled), novel stratum "
              "(the honest external number):")
        if "novel" in res_full and "novel" in res_sub:
            d_roc = res_sub["novel"]["roc_auc"] - res_full["novel"]["roc_auc"]
            d_pr = res_sub["novel"]["pr_auc"] - res_full["novel"]["pr_auc"]
            print(f"    ROC-AUC: {res_full['novel']['roc_auc']:.3f} -> "
                  f"{res_sub['novel']['roc_auc']:.3f}  (delta {d_roc:+.3f})")
            print(f"    PR-AUC:  {res_full['novel']['pr_auc']:.3f} -> "
                  f"{res_sub['novel']['pr_auc']:.3f}  (delta {d_pr:+.3f})")
    ...
    for label in ("LB strict", "LB permissive"):
        ...
        sub = merged[(merged.stratum == "novel") & merged.score.notna()]
        ci = s4.bootstrap_ci(sub.label.values, sub.score.values)
        print(f"\n  [{label}] novel stratum: n={len(sub)}, n_pos={int(sub.label.sum())}")
    ...
    if n_absent > 5:
        absent_hosts = hosts[hosts.sero_col.isna()].strain.tolist()
        print(f"  More than a handful -- reporting their metrics separately:")
        for label, path in INTERACTION_FILES.items():
            inter = pd.read_csv(path)
            sub = inter[inter.strain.isin(absent_hosts)]
            ...
            if len(merged) and len(set(merged.label)) > 1:
                ...
                print(f"    [{label}] absent-KL-type hosts only: n={m['n']} "
                      f"n_pos={m['n_pos']} ROC-AUC={m['roc_auc']:.3f} PR-AUC={m['pr_auc']:.3f}")
            else:
                print(f"    [{label}] absent-KL-type hosts only: n={len(merged)}, "
