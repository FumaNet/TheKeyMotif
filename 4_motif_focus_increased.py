...
for i, threshold in enumerate(thresholds):
    ...
    for train_index, test_index in logo.split(final_df, final_df['label'], final_df['group_loci']):
        ...
        # Preserve original interaction label
        train_df = train_df.rename(columns={"label": "interaction_label"})
        ...
        # Identify phage-host pairs with only one protein in training
        protein_counts = train_df.groupby(["accession", "phage_ID"])["protein_ID"].nunique()
        single_protein_pairs = protein_counts[protein_counts == 1].index
        train_df["single_protein"] = train_df.set_index(["accession", "phage_ID"]).index.isin(single_protein_pairs).astype(int)

        # Final label: motif-positive OR (single-protein and known interaction)
        train_df["label"] = ((train_df["motif_positive"] == 1) |
                             ((train_df["single_protein"] == 1) & (train_df["interaction_label"] == 1))).astype(int)

        # Sample negatives only from interaction_label == 0 and not labeled positive
        neg_candidates = train_df[(train_df["interaction_label"] == 0) & (train_df["label"] == 0)]
        neg_df = (
            neg_candidates.groupby(["accession", "phage_ID"])
            .apply(lambda x: x.sample(1, random_state=42))
            .reset_index(drop=True)
        )

        # Positives: everything labeled 1
        ...
        n_train_neg = (train_df["label"] == 0).sum()
        n_train_single_pos = train_df[(train_df["label"] == 1) & (train_df["single_protein"] == 1)].shape[0]
        print(f"TRAIN: {n_train_pos} positives ({n_train_single_pos} single-protein), {n_train_neg} negatives")
...
# === SAVE ALL RESULTS ===
with open("Results/4_AUCs_motif_focus_increased.pkl", 'wb') as f:
