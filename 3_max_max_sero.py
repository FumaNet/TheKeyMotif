...
for i, threshold in enumerate(thresholds):
    ...
    for train_index, test_index in logo.split(final_df, final_df['label'], final_df['group_loci']):
        ...
        # --- In-Fold Host Embedding Averaging, rendundant with the remove_duplicates above
        """train_df = train_df.groupby(['accession', 'phage_ID', "protein_ID"]).agg({
            **{col: 'mean' for col in train_df.columns if col.startswith(('sero_', 'virus_'))},
            'label': 'first',
            'protein_ID': 'first'
        }).reset_index()

        test_df = test_df.groupby(['accession', 'phage_ID', "protein_ID"]).agg({
